...
REPOS_SIM = ROOT / "repos" / "SimulacionClimatica" / "02_Modelado_Simulacion"
...
def _build_case_summary_table():
    ...
    for case_dir in cases:
        ...
        if not metrics:
            rows.append(f"| {name} | — | — | — | Sin métricas |")
            continue
        ...
        rows.append(f"| {name} | {edi:.3f} | {cr:.3f} | {c_pass}/5 | {status} |")
