"""
validate.py — 17_caso_oceanos

Hyperobject: Océanos / Ocean Heat Content proxy.
Observable: proxy OHC anual (World Bank).
"""
...
def make_synthetic(start_date, end_date, seed=101):
    """Señal mensual centrada en ~64 (OHC proxy) con tendencia + ciclo."""
    rng = np.random.default_rng(seed)
    dates = pd.date_range(start=start_date, end=end_date, freq="MS")
    n = len(dates)
    t = np.arange(n, dtype=float)

    # Forcing: tendencia creciente ~60-68 + oscilación tipo ENSO
    trend = 60.0 + 0.03 * t
    enso = 1.5 * np.sin(2 * np.pi * t / 48)  # ciclo ~4 años
    noise_f = rng.normal(0, 0.3, n)
    forcing = trend + enso + noise_f
    ...
    for i in range(1, n):
        obs[i] = obs[i - 1] + alpha_s * (forcing[i] - beta_s * obs[i - 1]) + rng.normal(0, 0.2)
