"""
data.py — Fuga de Cerebros Global
Genera datos con dinámica macro realista para dominio: Socioeconómico.
...
def fetch_data(cache_path=None, start_date=None, end_date=None, refresh=False):
    if cache_path and not refresh:
        if os.path.exists(cache_path):
            ...
            return df, {"source": "cache", "domain": "Socioeconómico"}
    ...
    alpha, beta = 0.18, 0.02
    x = 0.0
    signal = []
    for t in range(steps):
        f = forcing[t]
        dx = alpha * (f - beta * x)
        x = x + dx + rng.normal(0, 0.012)
        signal.append(x)
    ...
    return df, {"source": "synthetic_calibrated", "domain": "Socioeconómico", "snr": float(signal_std / meas_noise)}
