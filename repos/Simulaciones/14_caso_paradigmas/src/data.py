"""
data.py — Cambio de Paradigmas Científicos
Genera datos con dinámica macro realista para dominio: Epistémico.
...
def fetch_data(cache_path=None, start_date=None, end_date=None, refresh=False):
    if cache_path and not refresh:
        if os.path.exists(cache_path):
            ...
            return df, {"source": "cache", "domain": "Epistémico"}
    ...
    for t in range(steps):
        trend = 1.0 + 0.015 * t
        seasonal = 0.6 * math.sin(2 * math.pi * t / 15)
    ...
    return df, {"source": "synthetic_calibrated", "domain": "Epistémico", "snr": float(signal_std / meas_noise)}
