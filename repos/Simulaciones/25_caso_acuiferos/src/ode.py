...
def simulate_ode(params, steps, seed=4):
    ...
    recharge_coeff = float(params.get("ode_recharge", params.get("ode_alpha", 0.04)))
    extraction = float(params.get("ode_extraction", params.get("ode_beta", 0.06)))
    # Evapotranspiración: más alta cuando nivel freático cercano a superficie
    evapotransp = float(params.get("ode_evapotransp", 0.015))
    # Flujo lateral desde cuencas vecinas
    lateral_flow = float(params.get("ode_lateral", 0.01))
    ...
    for t in range(steps):
        ...
        # Extracción: saturación logística por capacidad de bombeo
        Q = extraction * H / (abs(H) + 1.0) if H > h_critical else extraction * 0.3
        # Flujo lateral: tiende a equilibrio regional
        F_lat = lateral_flow * (0.0 - H)
        # Subsidencia irreversible bajo umbral
        subsidence = 0.02 * max(0.0, h_critical - H) if H < h_critical else 0.0

        dH = R - ET - Q + F_lat - subsidence + random.gauss(0, noise_std)
        H += dH
        H = max(-5.0, min(H, 5.0))
