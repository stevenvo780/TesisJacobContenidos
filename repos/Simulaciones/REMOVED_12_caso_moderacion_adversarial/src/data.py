"""
data.py — Moderación Adversarial
Genera datos con dinámica macro realista para dominio: Digital.
...
def fetch_data(cache_path=None, start_date=None, end_date=None, refresh=False):
    if cache_path and not refresh:
        if os.path.exists(cache_path):
            ...
            return df, {"source": "cache", "domain": "Digital"}
    ...
    for t in range(steps):
        trend = 1.0 + 0.005 * t
        seasonal = 0.5 * math.sin(2 * math.pi * t / 6)
    ...
    return df, {"source": "synthetic_calibrated", "domain": "Digital", "snr": float(signal_std / meas_noise)}
