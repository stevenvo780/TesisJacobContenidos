...
def load_real_data(start_date, end_date):
    csv = os.path.join(os.path.dirname(__file__), "..", "data", "dataset.csv")
    df = pd.read_csv(csv, parse_dates=["date"])
    df = df[(df["date"] >= start_date) & (df["date"] <= end_date)].copy()
    # Interpolate NaN in drivers
    for c in ["journal_articles", "patent_residents"]:
        if c in df.columns:
...
def make_synthetic(start_date, end_date, seed=101):
    """Señal mensual continua centrada en ~2.1 (R&D % GDP) con tendencia + ciclos."""
    rng = np.random.default_rng(seed)
    dates = pd.date_range(start=start_date, end=end_date, freq="MS")
    n = len(dates)
    t = np.arange(n, dtype=float)

    # Forcing: tendencia lineal + ciclo largo (paradigma shift) + ruido
    trend = 2.0 + 0.003 * t
    cycle = 0.15 * np.sin(2 * np.pi * t / (n * 0.6))
    noise_f = rng.normal(0, 0.02, n)
    forcing = trend + cycle + noise_f

    # Observable: responde al forcing con lag + noise propio
    obs = np.zeros(n)
    obs[0] = forcing[0]
    for i in range(1, n):
        obs[i] = obs[i - 1] + 0.08 * (forcing[i] - 0.5 * obs[i - 1]) + rng.normal(0, 0.02)
