# Protocolo de Simulación — Mega-constelaciones Satelitales (Starlink)

## 1. Definición de escenario
- Objetivo: evaluar si proliferación de mega-constelaciones satelitales como proceso macro exhibe clausura operativa
  medible a través del protocolo C1–C5.
- Variable objetivo: N (número total de satélites operativos).
- Indicador empírico: `Conteo mensual de satélites Starlink operativos (cache local)`.
- Fuente: Datos públicos de despliegue Starlink (satélites operativos mensuales).
...
1. Obtener datos de Datos públicos de despliegue Starlink (satélites operativos mensuales).
...
- **ODE**: α (tasa de despliegue, ~0.18), β (decaimiento orbital, ~0.015).
