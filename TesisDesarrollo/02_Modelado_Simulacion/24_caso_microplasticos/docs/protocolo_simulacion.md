# Protocolo de Simulación — Contaminación por Microplásticos

## 1. Definición de escenario
- Objetivo: evaluar si acumulación global de microplásticos como proceso macro distribuido exhibe clausura operativa
  medible a través del protocolo C1–C5.
- Variable objetivo: M (índice global de concentración de microplásticos).
- Indicador empírico: `OWID plastic waste + ER.H2O.INTR.K3`.
...
1. Obtener datos de OWID (plastic waste) + World Bank (ER.H2O.INTR.K3 — recursos hídricos internos).
...
- **ODE**: α (tasa de ingreso, ~0.09), β (degradación, ~0.003 — muy lento).
